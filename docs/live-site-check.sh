...
section "Response headers (article)"
curl -sSI --max-time 20 "$BASE$ARTICLE" | grep -iE '^(HTTP|server|x-vercel|cf-|cache-control|content-security|x-robots|age|strict-transport)'

section "robots.txt"
curl -sS --max-time 20 "$BASE/robots.txt"

section "sitemap.xml"
curl -sS --max-time 20 "$BASE/sitemap.xml"

section "Article head tags"
html="$(curl -sS --max-time 20 "$BASE$ARTICLE")"
...
curl -sS --max-time 20 "$BASE/straipsniai/ai-klientu-aptarnavimas" | grep -q 'Pavyzdinis straipsnis' && echo "YES (placeholder is live)" || echo no
...
while IFS='|' read -r name ua; do
  code=$(curl -sS -o /dev/null --max-time 20 -A "$ua" -w "%{http_code}" "$BASE$ARTICLE")
  printf '%-18s %s\n' "$name" "$code"
done <<'EOF'
